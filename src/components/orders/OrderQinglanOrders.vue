        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
             <el-form-item prop="order" label="Order:">
                <el-select class="w-full" v-model="orderQinglanOrdersForm.order" placeholder="please select order" :disabled="orderList.length <= 0">
...
export default class OrderQinglanOrders extends Vue {
    ...
    searchOrders (value: any) {
      if (this.orderQinglanOrdersForm.barcode.trim().length < 3) {
        ...
        this.$emit('loadOn')
        return httpService
          .post('query/segmentOrderUpdater', { 'action': 'get_order', 'barcode': this.orderQinglanOrdersForm.barcode })
          .then((res: any) => {
            if (res.data.segment_orders.length > 0) {
              this.orderList = res.data.segment_orders
            } else {
              (this as any).alert({ type: 'error', msg: res.data.lims_response || 'Try another code' })
            }
          })

  ...
  <div class="tool-primer-coverage-wrapper">
    <div class="tool-rimer-coverage">
      <el-row :gutter="20" class="mb-20">
        ...
        <el-form
          :model="ToolStudyCollaboratorsForm"
          label-position="top"
          :rules="rules"
          ref="ToolStudyCollaboratorsForm"
        >
          ...
          <el-col :span="23">
            <p
              class="mb-20"
            >Drag names from the left box to the right-hand ‘collaborators’ box to assign them as a collaborator. Drag from right to left to remove collaborators. Study collaborators will have read and write access to all of the projects within this study.</p>
          </el-col>
      </el-row>
    </div>
    ...
    <el-row :gutter="20" class="mb-30 flex" v-if="dataToSend.collaborators.length">
      <el-col :span="12">
        ...
        <draggable
          class="min-h-450 el-card is-never-shadow p-10"
          v-model="investigators"
          group="collaborators"
        >
          <div
            class="inline-block w-1/2 p-1 cursor-pointer"
            v-for="(investigator, index) in investigators"
            :key="index"
          >
      ...
      <el-col :span="12">
        ...
        <draggable
          class="min-h-450 el-card is-never-shadow p-10"
          v-model="dataToSend.collaborators"
          group="collaborators"
        >
          <div
            class="inline-block w-1/2 p-1 cursor-pointer"
            v-for="(collaborator, index) in dataToSend.collaborators"
            :key="index"
          >
...
export default class ToolStudyCollaborators extends Vue {
  ...
  getInvestigators () {
    return httpService
      .post('query/collaboratorList', {
        study: this.ToolStudyCollaboratorsForm.studyName,
        scope: 'non_collaborators'
      })
      .then((res: any) => {
        this.investigators = res.data.rows.map((investigators : any) => investigators.id)
      })
      .catch((err: any) => {
        throw new Error(err)
      })
  ...
  getColaborators () {
    return httpService
      ...
      .then((res: any) => {
        ...
        this.dataToSend.collaborators = res.data.rows.map((colaborator : any) => colaborator.id)
      })
      .catch((err: any) => {
        throw new Error(err)
      })
